...
function App() {
  ...
  const fetchProfile = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8080/api'}/profile?userId=1');
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8080/api'}/history?userId=1');
      const data = await res.json();
      setHistories(data);
    } catch (err) {
  ...
  const saveProfile = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8080/api'}/profile', {
        method: 'POST',
